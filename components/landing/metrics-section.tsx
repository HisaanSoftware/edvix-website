...
const metrics = [
  { 
    ...
    sublabel: "across student cohorts",
  },
  { 
    value: 96, 
    suffix: "%", 
      ...
      ([entry]) => {
        if (entry.isIntersecting && !hasAnimatedRef.current) {
          ...
          const animate = (currentTime: number) => {
            ...
            const nextScrambleState = progress < 0.8;
            setCount((prev) => (prev === nextCount ? prev : nextCount));
            setIsScrambling((prev) => (prev === nextScrambleState ? prev : nextScrambleState));
            if (progress < 1) {
    ...
    <section ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span>Practice Test Engine</span>
          <span className="text-foreground">+ student & parent dashboards</span>
        </div>
      </div>
    </section>
  );
